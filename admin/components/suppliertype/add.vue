...
export default {
 ...
 data(){
     return{
         ...
         nameRule:[v=>!!v || 'Type name is required'],
         allowedRule:[v=>!!v || 'Enter allow categories'],
         snackbar:false,
         color:'',
         text:'',
         loading:false
     ...
     async submit(){
       if(this.$refs.form.validate())
       {
          ...
          this.loading=true
             try {
                 await this.$axios.post('api/admin/suppliertype',this.form).then((res)=>{
                        this.loading = false
                        this.color="success"
                        this.snackbar=true
                        this.text=res.data.message
                         this.$store.dispatch('suppliertypes/getTypes')
