...
export default {
 ...
 data(){
     return{
         ...
         form:{
              name:this.doc.name,
              locality:this.doc.locality,
              suppliertypeId:this.doc.suppliertypeId,
              pages:this.doc.pages,
              filesize:this.doc.filesize,
              filetype:this.doc.filetype,              
     ...
     async submit(){
       if(this.$refs.form.validate())
       {
          ...
          this.loading=true
             try {
                 await this.$axios.patch('api/admin/documents/'+this.doc.id,this.form).then((res)=>{
                        this.loading = false
                        this.color="success"
                        this.snackbar=true
                        this.text=res.data.message
                         this.$store.dispatch('documents/getDocuments',this.doc.suppliertypeId)
