<template>
  <div>
      <v-btn icon depressed color="primary" @click="add()"><v-icon>mdi-plus</v-icon></v-btn>
      ...
      <v-dialog v-model="addPermModel" width="900">
       ...
       <v-card>
           ...
           <v-card-text>
               ...
               <v-row>
                <v-col>
                    <v-select v-model="form.procuremententityId" :rules="entityRule" outlined label="Entity" :items="entities" item-text="name" item-value="id" />
                </v-col>
                <v-col>
...
export default {
 ...
 data(){
    ...
    checkproduct(name){
     ...
     entities(){
         return this.$store.state.procuremententity.data
     },
     sections(){
         return this.$store.state.sections.sections
     },
     categories(){
         let data =  this.$store.state.category.categories
         if(this.search){
